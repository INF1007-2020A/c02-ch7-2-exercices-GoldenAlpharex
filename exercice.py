...
def build_print_note_name_callback(midi_to_name):
	# Fonction locale
	# Soit message MIDI midi_msg, midi_msg.type donne le type, midi_msg.note done la note.
	
	# Dans mon callback :
	def callback(midi_msg):
		# Si j'ai un note_on et une vélocité > 0, alors on affiche le nom de la note associée au numéro (grâce au dict midi_to_name)
